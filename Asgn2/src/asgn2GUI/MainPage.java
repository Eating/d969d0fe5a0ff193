...
public class MainPage {
	...
	/**
	 * Build a string for drivers to read,
	 * return specific information for drivers
	...
	private String buildDriversText(){
		...
		if (!theTrain.trainCanMove()){
			ret += " - Train cannot move";
		}
		return ret;
	}
	
	/**
	 * Build a string for conductors to read,
	 * return specific information for conductors
	...
	private void disableBoardingArea(){
		...
		spinnerToAlight.setEnabled(false);
	}
	...
	private void enableBoardingArea(){
		...
		spinnerToAlight.setEnabled(true);
		alightNumberModel.setMaximum(passengersOnBoard);
	}
	...
	/**
	 * Reconfigure the data and GUI layouts after changing composition of the train
	 */
	...
	private void makeLabels(){
		...
		if (r == null){
			...
			tmpPanel.setBorder(new BevelBorder(BevelBorder.RAISED, null, null, null, null));
			...
			if (r.getClass().getName() == "asgn2RollingStock.Locomotive"){
				...
				progressBar.setValue((int) (((double)totalWeight/(double)power)*100));
				tmpPanel.setLayout(new MigLayout("", "[146px]", "[61px][14px]"));
				...
				progressBar.setValue(
						(int) ((double)((PassengerCar)r).numberOnBoard()/
						(double)((PassengerCar)r).numberOfSeats()
						*100));
				tmpPanel.setLayout(new MigLayout("", "[146px]", "[61px][14px]"));
				tmpPanel.add(label, "cell 0 0,grow");
				tmpPanel.add(progressBar, "cell 0 1,alignx left,aligny top");
	...
	public MainPage() {
		...
		driverPanel.setBorder(new TitledBorder(null, "Driver", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		conductorPanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Conductor", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		btnBoard.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				int toBoard = Integer.parseInt((spinnerToBoard.getValue().toString()));
				RollingStock r = theTrain.firstCarriage();
				int remaining = (totalSeats - passengersOnBoard);		//remaining == seats available
				remaining = remaining > toBoard? toBoard : remaining;	//remaining == number to board
				passengersRefused += toBoard - remaining;
				passengersOnBoard += remaining;
		
				r = theTrain.nextCarriage();
				while (r != null && r.getClass().getName() == "asgn2RollingStock.PassengerCar"
						&& remaining > 0){
					if (remaining > ((PassengerCar)r).numberOfSeats()-((PassengerCar)r).numberOnBoard()){
						try{
							int tmpBoarding = ((PassengerCar)r).numberOfSeats()-((PassengerCar)r).numberOnBoard();
		...
		JLabel lblToAlight = new JLabel("To alight:");
		lblToAlight.setFont(new Font("Arial", Font.PLAIN, 14));
		lblToAlight.setBounds(262, 149, 74, 15);
		conductorPanel.add(lblToAlight);
		
		alightNumberModel.setMaximum(0);
		alightNumberModel.setMinimum(0);
		spinnerToAlight = new JSpinner(alightNumberModel);
		spinnerToAlight.setBounds(328, 146, 74, 22);
		conductorPanel.add(spinnerToAlight);
		
		btnAlight = new JButton("Alight");
		btnAlight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {

				int toAlight = Integer.parseInt((spinnerToAlight.getValue().toString()));
				RollingStock r = theTrain.firstCarriage();
				r = theTrain.nextCarriage();
				while (r != null && r.getClass().getName() == "asgn2RollingStock.PassengerCar"
						&& toAlight > 0){
					if (toAlight > ((PassengerCar)r).numberOnBoard()){
						try{
							((PassengerCar)r).alight(((PassengerCar)r).numberOnBoard());
							toAlight -= ((PassengerCar)r).numberOnBoard();
						...
						try{
							((PassengerCar)r).alight(toAlight);
							toAlight = 0;
						}
						catch (TrainException e1){
							e1.printStackTrace();
		...
		btnAlight.setBounds(262, 187, 140, 30);
